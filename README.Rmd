...
C/TinyCC path, not as the compiler architecture. C, Fortran, and Rtinycc paths
now consume one backend-neutral `TccqLoopNest` — a SAC-style with-loop carrying
typed axes, affine accesses, and an optional reducer — for elementwise maps,
full and per-axis reductions, `%*%` contractions, and interior slices, but the
...
full-domain and per-axis reductions, `%*%` matrix-vector and matrix-matrix
contractions, and 1-D interior stencils now pass through one typed loop nest
into generated C, Fortran, and TinyCC-JIT code that compiles and runs. The
remaining probes — general indexing, control flow, `raw`/`buffer` bridges,
apply-family folds, and the composite targets — are still expected to fail
with structured diagnostics. Making each failure move deeper through the same
