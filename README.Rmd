...
elementwise expressions over declared `integer` and `double` values for `+`,
`-`, `*`, `/`, unary negation, `sqrt`, and `exp`. It returns a
`TccqLoweringPlan`, then embeds the plan into `TccqProgram` as values, regions,
fusion groups, and storage facts. Each operation value carries a
`TccqResolvedOp`, and the generated fusion group derives its target and effect
from those resolved operations. This is not a general legality pass and it is
not the place where new language coverage should sprawl.
