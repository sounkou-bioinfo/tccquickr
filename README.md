...
Registries can add rendered operations without touching the printers.
Every neutral operation expression owns its complete
`TccqLoweredOperation` and typed effect; it does not duplicate the
selected implementation or hide pass facts in expression attributes.
...
completion rather than fabricated read/write effects. A nested
...
graph-consistent target types, and initialization dominance. `for`,
