...
structured refusals. The composite name is gone before expression trees,
fusion, loop nests, or source backends see the program. Every neutral
operation expression then owns its complete `TccqLoweredOperation` and
typed effect; it does not duplicate the selected implementation or hide
pass facts in expression attributes.
