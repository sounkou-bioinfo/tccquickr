...
tccq_c_expr_length <- function(expr, sym) {
  ...
  if (identical(expr$tag, "slice_range")) {
    tccq_abort("slice length as inline expression is not supported; materialize/bind it first")
  }
  vec_vars <- tccq_ir_vector_vars(expr)
  if (length(vec_vars) == 1L) {
...
tccq_c_emit_kernel <- function(module, sym) {
  ...
  c(
    ...
    switch(
      kernel$tag,
      scalar_kernel = tccq_c_emit_kernel_scalar(kernel, sym),
      materialize = tccq_c_emit_kernel_materialize(kernel, sym),
      fold = tccq_c_emit_kernel_fold(kernel, sym),
      kernel_program = tccq_c_emit_kernel_program(kernel, sym),
      tccq_abort("unknown kernel tag: ", kernel$tag)
    )
  )
}

tccq_c_emit_kernel_scalar <- function(kernel, sym) {
...
tccq_c_emit_kernel_materialize <- function(kernel, sym) {
  mode <- kernel$type$mode
  out_sexp <- tccq_sexptype_for_mode(mode)
...
tccq_c_emit_kernel_fold <- function(kernel, sym) {
...
tccq_c_emit_kernel_program <- function(kernel, sym) {
  c(
    unlist(lapply(kernel$stmts, tccq_c_emit_stmt, sym = sym), use.names = FALSE),
    switch(
      kernel$result_kernel$tag,
      scalar_kernel = tccq_c_emit_kernel_scalar(kernel$result_kernel, sym),
      materialize = tccq_c_emit_kernel_materialize(kernel$result_kernel, sym),
      fold = tccq_c_emit_kernel_fold(kernel$result_kernel, sym),
      tccq_abort("unsupported program result kernel: ", kernel$result_kernel$tag)
    )
  )
}

tccq_c_emit_stmt <- function(stmt, sym) {
  switch(
    stmt$tag,
    bind = tccq_c_emit_stmt_bind(stmt, sym),
    store_index = tccq_c_emit_stmt_store_index(stmt, sym),
    store_range = tccq_c_emit_stmt_store_range(stmt, sym),
    tccq_abort("unsupported statement tag: ", stmt$tag)
  )
}

tccq_c_emit_stmt_bind <- function(stmt, sym) {
  s <- sym[[stmt$name]]
  ...
  if (identical(stmt$value$tag, "slice_range")) {
    return(tccq_c_emit_bind_slice_range(stmt, sym))
  }
  ...
  c(
    ...
    "++tccq_nprotect;",
    paste0(tccq_c_scalar_type_for_mode(type$mode), " *", s$ptr, " = ", tccq_c_rw_accessor(type$mode), "(", s$sexp, ");"),
    paste0("for (R_xlen_t i = 0; i < ", s$len, "; ++i) {"),
    paste0("  ", s$ptr, "[i] = (", tccq_c_scalar_type_for_mode(type$mode), ")(", elem, ");"),
    "}"
  )
}

tccq_c_emit_stmt_store_index <- function(stmt, sym) {
  s <- sym[[stmt$name]]
  if (is.null(s) || !identical(s$kind, "local")) {
...
tccq_c_emit_stmt_store_range <- function(stmt, sym) {
  s <- sym[[stmt$name]]
  if (is.null(s) || !identical(s$kind, "local")) {
...
tccq_c_emit_slice_range_indices <- function(node, sym, prefix) {
  if (!identical(node$x$tag, "var")) {
    tccq_abort("slice_range currently supports direct variable base only")
  }
...
tccq_c_emit_bind_slice_range <- function(stmt, sym) {
  s <- sym[[stmt$name]]
  mode <- stmt$type$mode
  ctype <- tccq_c_scalar_type_for_mode(mode)
...
tccq_c_emit_slice_range_expr <- function(node, sym, idx = NULL) {
  if (is.null(idx)) {
    tccq_abort("slice_range requires an element index in expression emission")
  }
  if (!identical(node$x$tag, "var")) {
    tccq_abort("slice_range currently supports direct variable base only")
  }
