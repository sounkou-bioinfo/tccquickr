# test_tccq_runtime_errors.R

index_read_fn <- function(x, i) {
  declare(type(x = double(NA)), type(i = integer()))
  x[i] + 1
}
compiled_index_read <- tccq_compile(index_read_fn)
...
compiled_length_mismatch <- tccq_compile(length_mismatch_fn)
expect_error(
...
compiled_slice <- tccq_compile(slice_fn)
...
compiled_range_assign <- tccq_compile(range_assign_runtime_fn)
expect_error(
...
compiled_integer_scalar <- tccq_compile(integer_scalar_fn)
